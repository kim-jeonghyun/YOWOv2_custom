...
# 로그 파일의 이름을 오늘 날짜로 설정합니다.
log_file_name = datetime.datetime.now().strftime("%Y-%m-%d-%H:%M:%S") + ".log"

# 로그 파일을 엽니다.
logging.basicConfig(filename=os.path.join("log",log_file_name), level=logging.INFO)
...
def parse_args():
    ...
    parser.add_argument('--vis_data', action='store_true', default=True,
                        help='use tensorboard')
...
def train():
    ...
    # path to save model
    path_to_save = os.path.join(args.save_folder, args.dataset, args.version, datetime.datetime.now().strftime("%Y-%m-%d-%H:%M:%S"))
    os.makedirs(path_to_save, exist_ok=True)
    logger.info("Save path: {}".format(path_to_save))
...
def eval_one_epoch(args, model_eval, evaluator, epoch, path_to_save):
    ...
    if distributed_utils.is_main_process():
        ...
        torch.save({'model': model_eval.state_dict(),
                    ...
                    'args': args}, 
                    checkpoint_path)                      
        logger.info("Save model to {}".format(checkpoint_path))
